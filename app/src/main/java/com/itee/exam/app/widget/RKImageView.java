...
public class RKImageView extends ImageView {
	...
	public RKImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public RKImageView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray array = context.obtainStyledAttributes(attrs,
					R.styleable.RKImageView);
			borderColor = array.getColor(R.styleable.RKImageView_border_color,
					borderColor);
			borderWidth = array.getDimensionPixelOffset(
					R.styleable.RKImageView_border_width, borderWidth);
			pressAlpha = array.getInteger(R.styleable.RKImageView_press_alpha,
					pressAlpha);
			pressColor = array.getColor(R.styleable.RKImageView_press_color,
					pressColor);
			radius = array.getDimensionPixelOffset(
					R.styleable.RKImageView_radius, radius);
			shapeType = array.getInteger(R.styleable.RKImageView_shape_type,
					shapeType);
			array.recycle();
		}
	...
	/**
	 * 这里是参考其他开发者获取Bitmap内容的方法， 之前是因为没有考虑到 Glide 加载的图片 导致drawable 类型是属于
